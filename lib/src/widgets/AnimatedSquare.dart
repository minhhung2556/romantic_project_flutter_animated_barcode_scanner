...
class AnimatedSquareState extends State<AnimatedSquare> with TickerProviderStateMixin {
  ...
  void changeToOffset(List<Offset> newOffset) {
    ...
    // TODO end animation
    // controller.forward().whenComplete(() => widget?.onScan(scanResult));
  }
  ...
  TickerFuture revertOffset() {
    ...
    final List<Offset> newOffset = [Offset(left, top), Offset(right, top), Offset(right, bottom), Offset(left, bottom)];
    setState(() {
      controller.stop();
  ...
  void initAnimation() {
    ...
    from.clear();
    from.addAll([Offset(left, top), Offset(right, top), Offset(right, bottom), Offset(left, bottom)]);
    offsets = from;
    ...
    to.clear();
    to.addAll([Offset(left, top), Offset(right, top), Offset(right, bottom), Offset(left, bottom)]);
        ...
        child: CustomPaint(
          painter: _SquarePainter(
            ...
            borderColor: widget.squareBorderColor ??
                ((widget.squareColor != null) ? widget.squareColor.withOpacity(1) : Colors.blue),
